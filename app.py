...
dfTabelaGastos_2019['gastos_2019'] = dfTabelaGastos_2019['gastos_2019'].apply(lambda x: round(x,2) if not pd.isnull(x) else 0)
...
def gerar_mapa(tipografico, anos_ideb, tipodados, anos_universalizacao = 0):
    ...
    if tipografico == "ideb":
        ...
        if tipodados == "distrito":
            ...
            fig.update_layout(margin={"r":0,"t":50,"l":0,"b":0}, showlegend=False, height=513, title="Distribuição IDEB por Distritos")
            ...
            if tipodados == "subprefeitura":
                ...
                fig.update_layout(margin={"r":0,"t":50,"l":0,"b":0}, showlegend=False, height=513, title="Distribuição IDEB por Subprefeituras")
        ...
        if tipografico == "universalizacao":
            if anos_universalizacao == 2019:
                ...
                geodf['text'] = geodf['ds_nome'] + ':<br>Taxa(%):' \
                                + geodf['universalizacao_2019'].apply(
                    lambda x: str(round(x, 2)) if not x==0 else 'Não se aplica')
                ...
                fig.update_geos(fitbounds="locations", visible=False)
                fig.update_layout(margin=dict(l=0, r=0, t=50, b=0), showlegend=False, height=513, title="Universalização da Educação Infantil (2019)")
            else:
                ...
                fig.update_geos(fitbounds="locations", visible=False)
                fig.update_layout(margin=dict(l=0, r=0, t=50, b=0), showlegend=False, height=513, title="Universalização da Educação Infantil (2020)")
            ...
            if tipografico == "gastos":
                ...
                geodf['text'] = geodf['ds_nome'] + ':<br>Nota média:' \
                    ...
                    lambda x: '{:,.2f}'.format(float(str(round(x, 2)))) if not pd.isna(x) or x !=0  else 'Não se aplica')
                ...
                fig.update_geos(fitbounds="locations", visible=False)
                fig.update_layout(margin=dict(l=0, r=0, t=50, b=0), showlegend=False, height=513, title="Gastos por Distrito (2019)")
                        ...
                        html.Div(style={'backgroundColor': colors['background'], 'color': colors['text']},
                                ...
                                dash_table.DataTable(id="tabelagastos2019",
                                                     ...
                                                     style_cell={'backgroundColor': colors['table_cell'],
                                                                 ...
                                                                 'textAlign': 'left', 'width': '85px', 'width': '85px',
                                                                 'width': '85px',
                                                                 'whiteSpace': 'normal', 'fontSize': 13,
                                                                 'font-family': 'arial'},
                                                     columns=[{'id': c, 'name': c, 'type': 'numeric', 'format': Format(
                scheme=Scheme.fixed,
                precision=2,
                group=Group.yes,
                groups=3,
        ...
        dbc.Col(style={'backgroundColor': colors['background'], 'color': colors['text']}, children=[
            ...
            dbc.Collapse(
                ...
                dbc.CardBody([
                    ...
                    dbc.Collapse(
                        dbc.Card(style={'margin-top': 20, 'backgroundColor': colors['background'], 'color': colors['topic_text']},
                                 children=[
    ...
    if indicadores_educacao is not None:
        if indicadores_educacao =="ideb":
            ...
            if indicadores_educacao == "idep":
                ...
                fig3.update_layout(margin=dict(l=0, r=0, t=50, b=0), autosize=True, showlegend=True, title="Escolas que atingiram a meta do IDEP <br> Anos Iniciais (2019)")

                labels = ['Não Atingiram', 'Atingiram']
                values = [65.3, 34.7]

                fig4 = go.Figure(data=[go.Pie(labels=labels, values=values)])
                fig4.update_layout(margin=dict(l=0, r=0, t=50, b=0), autosize=True, showlegend=True, title="Escolas que atingiram a meta do IDEP <br> Anos Finais (2019)")
                    ...
                    if indicadores_educacao == "universalizacao":
                        ...
                        if indicadores_educacao == "gastos":
                            ...
                            if indicadores_educacao == "eja":
                                ...
                                #fig2 = go.Figure(data=[go.Pie(labels=labels, values=values)])
                                #fig2 = px.pie(dfEjaGenero, values='Gênero', names='Quantidade', title='Population of European continent')
                                ...
                                dfSlider  = dfEjaMatriculas.loc[(dfEjaMatriculas['Ano'] >= sliderEja[0]) & (dfEjaMatriculas['Ano'] <= sliderEja[1])]
                                ...
                                labels = ['Amarelo', 'Branco', 'Indígena', 'Não declarada', 'Parda', 'Preta', 'Recusou informar']
        ...
        if indicadores_orcamento is not None:
            ...
            return (go.Figure(), go.Figure(), go.Figure(), go.Figure(), collapsedivdanos, colapseddivistritossubpreituras, card_universalizacao, divEsquerdaSup, divEsquerdaInf, divEsquerdaInf2, divGrafDireita, info, divInfo, divSlider, info_header, collapseGraficosEsquerda, collapseGraficosDireita, card_Apresentacao_Direita, collapseTabelaDistrito, collapseTabelaSubprefeitura, collapseTabelaGastos2019, collapseTabelaEja)


    return (fig, fig2, fig3, fig4, collapsedivdanos, colapseddivistritossubpreituras, card_universalizacao, divEsquerdaSup, divEsquerdaInf, divEsquerdaInf2, divGrafDireita, info, divInfo, divSlider, info_header, collapseGraficosEsquerda, collapseGraficosDireita, card_Apresentacao_Direita, collapseTabelaDistrito, collapseTabelaSubprefeitura, collapseTabelaGastos2019, collapseTabelaEja)
