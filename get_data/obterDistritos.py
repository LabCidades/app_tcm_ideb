...
def distritos(path_salvo, dfIdebIniciais, dfIdebFinais):
    ...
    if geodfDistritos is not None:
        ...
        dfGastos['gastos_2019'] = dfGastos['gastos_2019'].apply(
            lambda x: round(x, 2) if not pd.isnull(x) else 0)
        dfGastos['EDU_PER_CAPITA_anual_2020'] = pd.to_numeric(dfGastos['EDU_PER_CAPITA_anual_2020'], errors='coerce')
        dfGastos['EDU_PER_CAPITA_anual_2020'] = dfGastos['EDU_PER_CAPITA_anual_2020'].apply(
            lambda x: round(x, 2) if not pd.isnull(x) else 0)
        ...
        dfGastos['REMUNERACAO_BRUTA'] = dfGastos['REMUNERACAO_BRUTA'].apply(
            lambda x: round(x, 2) if not pd.isnull(x) else 0)
        dfGastos['EDU_VALOR_TOTAL_ANUAL_2020'] = dfGastos['EDU_VALOR_TOTAL_ANUAL_2020'].str.replace(',', '.').astype(float)
